...
virt_viewer_deactivated(VirtViewerApp *app)
{
	...
	if (priv->reconnect) {
		...
		virt_viewer_app_set_status(app, "Waiting for guest domain to re-start");
...
virt_viewer_initial_connect(VirtViewerApp *app)
{
	...
	virt_viewer_app_set_status(app, "Finding guest domain");
	dom = virt_viewer_lookup_domain(self);
	if (!dom) {
		if (priv->waitvm) {
			virt_viewer_app_set_status(app, "Waiting for guest domain to be created");
	...
	virt_viewer_app_set_status(app, "Checking guest domain status");
	...
	if (info.state == VIR_DOMAIN_SHUTOFF) {
		virt_viewer_app_set_status(app, "Waiting for guest domain to start");
	} else {
		...
		if (ret < 0) {
			if (priv->waitvm) {
				virt_viewer_app_set_status(app, "Waiting for guest domain to start server");
